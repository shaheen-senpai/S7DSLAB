...
class ChatClient:
    ...
    def gui_init(self):
        ...
        self.send_button.pack(side = "left", expand = True, fill = "both")

        self.reverse_button = tk.Button(self.root, text="Reverse", command=self.reverse_message)
        self.reverse_button.pack(side = "left", expand = True, fill = "both")
    ...
    def reverse_message(self):
        ...
        if message:
            ...
            self.sock.send(bytes(f"{self.username}: {reversed_message}", 'utf-8'))
            self.display_message(f"{self.username} (Reversed): {reversed_message}")
